...
"""
Pancosmic_Datain.py scrapes the html code from the JPL PDS website for the Mars
Exploration Rover of the users choosing and for the preferred range of solar
days the user chooses. These script then filters the html code for the PDS image
file name and its respective URL and then writes that information to the
Pancosmic_Files.txt text file. This text file will be entered into a Pandas
dataframe in the PanCosmic_DataProcess.py script. PanCosmic_Datain.py is an
optional script if the user has already scraped the JPL website for the rover
and range of days they are interested in studying.
"""
import os
import urllib2
import re
...
file_check = os.path.isfile('Pancosmic_Files.txt')

if file_check is False:
    print('Pancosmic_Files.txt does not exist. Please choose 0 below:')
else:
    print('Pancosmic_Files.txt exists')
print('Have you already selected the rover and range of days for data you wish to collect? (1/0)')
response1 = raw_input()
response = int(response1)
if response == 1 and file_check is True:
    sys.exit()
elif response == 0:
    ...
    os.system('./PanCosmic_Datain.py')
print('Coarse Parsing completed....')
